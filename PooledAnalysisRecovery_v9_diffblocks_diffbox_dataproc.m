...
% Nathan V-C  7/2015
% -----------------------
% This file is for recovering pooled analysis for days where theclre was 
...
sess_start=input('Enter number of starting session for block 1: ');
numsess=input('Enter number of ending session for block 1: ');
display('-----')

blck2 = input('Is there another block with different ear tags? (y/n)','s');

if blck2 == 'y'
    
    display('Enter block 2 mice')
    display('-----')
    display('Enter tags for all mice who ran in a group of four, all mice must have run the same sessions, and numbers must be entered exactly as in original acquisition : ')
    MouseTag1_2=input('Enter MouseTag for Box 1 (0 for no mouse) : ', 's');
    MouseTag2_2=input('Enter MouseTag for Box 2 (0 for no mouse) : ', 's');
    MouseTag3_2=input('Enter MouseTag for Box 3 (0 for no mouse) : ', 's');
    MouseTag4_2=input('Enter MouseTag for Box 4 (0 for no mouse) : ', 's');
    MouseTag5_2=input('Enter MouseTag for Box 5 (0 for no mouse) : ', 's');
    
    % must start 1 after end of block 2
    sess_start_2=input('Enter number of starting session for block 2: ');
    numsess_2=input('Enter number of ending session for block 2: ');
    display('-----')
    
end

group_ind(1:(numsess-sess_start+1))=1;
group_ind((numsess-sess_start+2):(numsess-sess_start+2)+(numsess_2-sess_start_2))=2;
group_ind

% enter 1 for each box to actually run analysis for, if enter zero, skips
% that box/mouse, 
% Must run one mouse at time, indicate location of mouse in group 1 & group
% 2
run_analysis_1=[0 0 0 0 1];
run_analysis_2=[1 0 0 0 0];
...
if blck2 == 'y'
    %block2 tags
    MouseTags_2={MouseTag1_2 MouseTag2_2 MouseTag3_2 MouseTag4_2 MouseTag5_2};
    % Boxes that were actually run for block 2
    ActiveBoxes_2=find(~strcmp(MouseTags_2,'0')==1);
    % cell containing which mice are actually running for block 2
    ActiveTags_2=MouseTags(ActiveBoxes_2);
    % Tag numbers in a string for saving files that apply to all mice for block
    % 2
    ActiveTagsStr_2=strjoin(MouseTags_2,'_');
end
...
if DataVersion==0;
    ...
    for missingsess=sess_start_2:numsess_2
        %findfile_temp={filelist{strmatch(['_AllBoxesRawData_Session_' num2str(missingsess) '_' ActiveTagsStr '_' SessionTag],filelist)}};
        findfile_temp=filelist(find(~cellfun(@isempty,regexp(filelist,['_AllBoxesRawData_Session_' num2str(sess_start+k-1) '_' ActiveTagsStr_2 '_' SessionTag]))==1));
        if length(findfile_temp)==1
            % Possible this is wrong format for concatenating this path name
            rawdatafiles_all{k}=[datadir findfile_temp{1}];
        elseif isempty(findfile_temp)
...
%---------------------------------------
runsave_pooled_analysis_5box_toneonly_blank_regen_diffblocksbox(rawdatafiles_all, ActiveBoxes, ActiveBoxes_2, MouseTags, time_params, datadir, datasavedir, figdir_pool, SessionTag, TypeTrial, [], [], [], email, timetag_ids, run_analysis_1, run_analysis_2, group_ind)
