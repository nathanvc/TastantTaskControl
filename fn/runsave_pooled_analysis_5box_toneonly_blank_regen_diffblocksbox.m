...
function runsave_pooled_analysis_5box_toneonly_blank_regen_diffblocksbox(rawdatafiles_all, ActiveBoxes_1, ActiveBoxes_2, MouseTags, time_params, datadir, datasavedir, figdir_pool, SessionTag, TypeTrial, NotebookTag, Notebook, matlabfile, email, timetag_ids,run_analysis_1, run_analysis_2, group_ind)

numbox_1=length(ActiveBoxes_1);
numbox_2=length(ActiveBoxes_2);
include_box_1=find(run_analysis_1==1)
include_box_2=find(run_analysis_2==1)

% MUST RUN ANALYSIS ONE BOX AT A TIME WHEN COMBINING IN THIS WAY
for i=1
    for session=1:length(rawdatafiles_all)
        session
        if group_ind(session) == 1
            %if ismember(ActiveBoxes_1(i),include_box_1),
                % Identify mouse and box
                %-------
            Box=include_box_1(i)
            MouseTag=MouseTags{Box}
            run_ind = 1
        elseif group_ind(session) == 2
            Box=include_box_2(i)
            %MouseTag=MouseTags{Box} % already defined
            run_ind = 1
        end
